	...
	if(client.itemInformation(args[1], "id")){
		...
		message.awaitReactions(client.filter, { max: 1, time: 60000, errors: ['time'] }).then(collected => {
			...
			if (reaction.emoji.name === '✅') {
				iManage.giveToPlayerItem(message.author.id, args[1],  (args[2]*-1 || -1));
		        return message.channel.send({embed: {color: "GREEN", description: `:white_check_mark: <@${message.author.id}>, L'item **${client.itemInformation(args[1], "name")}**  L'item a été retiré!`}});
			} else {
